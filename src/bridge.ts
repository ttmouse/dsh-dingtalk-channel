...
import { ConversationSessions, conversationKey, type SessionLadder } from './session.js'
import type {
  HostAgent,
  HostAgentOptions,
  HostAgentPresets,
...
import { CHAT_COMMANDS, parseCommand, type CommandEnv } from './commands.js'
import { matchApprovalDecision } from './approval-reply.js'
import { resolveAuthorization, refuseApproval, refuseMessage, type Authorization } from './authorization.js'
...
/** One pending chat approval, awaiting a text reply in its chat. */
...
/** What one agent joins, resolved once and shared by every rung of its ladder. */
 ...
 */
function boundArgs(text: string): string {
 ...
 */
export function chatUserMessage(msg: InboundMessage): HostUserMessage {
  ...
  return Object.freeze({
    id: randomUUID(),
    role: 'user',
    content: Object.freeze([{ type: 'text' as const, text: spoken }]),
...
export function installBridge(
  ...
  const bySession = new Map<string, ReturnType<typeof createChatRenderer>>()
  ...
  const emotionBySession = new Map<string, string>()
  ...
  const authorization: Authorization = resolveAuthorization(config)

  const rememberMessage = (messageId: string): void => {
    const now = Date.now()
    if (seenMessages.size >= DEDUP_MAX_IDS) {
      for (const [id, at] of seenMessages) {
        if (now - at > DEDUP_WINDOW_MS) seenMessages.delete(id)
      }
      if (seenMessages.size >= DEDUP_MAX_IDS) {
        const oldest = [...seenMessages.entries()].sort((a, b) => a[1] - b[1]).slice(0, DEDUP_MAX_IDS / 4)
        for (const [id] of oldest) seenMessages.delete(id)
      }
    }
  ...
  const modelSelection = (): HostAgentOptions => {
    ...
    if (defaults === undefined) {
      throw new Error(
        'dsh-dingtalk-channel: no model configured — set config.provider/model or compose the agentDefaultModel service',
      )
    }
  ...
  const sessionIdForKey = (key: string): string => {
    ...
    if (route === undefined || route === '') routeBySession.delete(id)
    else routeBySession.set(id, splitRoute(route))
    return id
  }
  ...
  const webhookFor = (chatId: string): string | undefined => webhooks.get(chatId)

  const sendMarkdown = async (chatId: string, text: string): Promise<void> => {
    const webhook = webhookFor(chatId)
    if (webhook === undefined) throw new Error('no reply webhook cached for this conversation')
  ...
  const runHostCommandLine = async (
    command: { name: string; args: string },
    agent: HostAgent,
  ): Promise<string> => {
    const hosted = ctx.get('commands') as HostCommands | undefined
    if (hosted === undefined) return `未知命令 /${command.name}（/help 查看可用命令）`
    const known = hosted.list(agent).some(descriptor => descriptor.name === command.name)
    if (!known) return `未知命令 /${command.name}（/help 查看可用命令）`
    const outcome = await hosted.execute(agent, `/${command.name} ${command.args}`.trim(), commandSignal())
    if (outcome === undefined) return `未知命令 /${command.name}`
  ...
  const askViaChat = async (
    ...
    for (const pending of pendingApprovals.values()) {
      if (pending.chatId === chatId) return next()
    }
    const id = randomUUID()
  ...
  const trySettleApproval = async (msg: InboundMessage): Promise<boolean> => {
    const decision = matchApprovalDecision(msg.text)
    if (decision === undefined) return false
    for (const [id, pending] of [...pendingApprovals]) {
      if (pending.chatId !== msg.chatId) continue
  ...
  const handleMessage = async (msg: InboundMessage): Promise<void> => {
    ...
    if (await trySettleApproval(msg)) return
    ...
    if (command !== undefined && CHAT_COMMANDS.has(command.name)) {
      ...
      const env: CommandEnv = {
        ...
        stop: () => {
          ...
          if (live !== undefined) live.cancel('user')
        },
        reset: async () => {
      ...
      try {
        ...
        if (reply !== '') await sendMarkdown(msg.chatId, reply).catch(reportSendFailure)
      } catch (error) {
  ...
  ctx.on('approval/request', (request, next) => {
    if (bySession.get(request.agent.session.id) === undefined) return next()
...
function splitRoute(route: string): HostAgentOptions {
