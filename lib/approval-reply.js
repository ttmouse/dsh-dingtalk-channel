 ...
 */
export function matchApprovalDecision(text) {
    const t = text.trim().toLowerCase();
    if (t.length > 12)
        return undefined;
    if (/^(允许一次|允许|同意|批准|approve|allow|yes|y)$/.test(t))
        return 'allow';
    if (/^(拒绝|不同意|取消|reject|deny|no|n)$/.test(t))
        return 'reject';
    return undefined;
}
//# sourceMappingURL=approval-reply.js.map